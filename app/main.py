...
def main_screen():
    ...
    if 'edit_participant' not in st.session_state:
        st.session_state['edit_participant'] = None

    if st.session_state['edit_participant']:
        participant = st.session_state['edit_participant']
        st.title("Edit Participant")
        gender = st.selectbox("Gender*", ["Male", "Female"], index=["Male", "Female"].index(participant["gender"]))
        first_name = st.text_input("First Name*", value=participant["first_name"])
        last_name = st.text_input("Last Name*", value=participant["last_name"])
        email = st.text_input("Email*", value=participant["email"])
        phone = st.text_input("Phone*", value=participant["phone"])
        ...
        if st.button("Save Changes"):
            if not first_name or not last_name or not email or not phone:
                ...
                response = requests.put(f"{API_URL}/participant/{participant['_id']}", json=updated_data)
                if response.status_code == 200:
                    st.success("Participant updated successfully")
                    st.session_state['edit_participant'] = None
                    st.rerun()
                else:
                    st.error("Failed to update participant")
    else:
            ...
            with st.expander(f"{participant['last_name']}, {participant['first_name']} - {format_time(time_value)}"):
                ...
                with col1:
                    if st.button("Edit", key=f"edit_{participant['_id']}"):
                        st.session_state['edit_participant'] = participant
                        st.rerun()
